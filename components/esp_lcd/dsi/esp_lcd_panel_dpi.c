...
static bool dma_trans_done_cb(dw_gdma_channel_handle_t chan, const dw_gdma_trans_done_event_data_t *event_data, void *user_data)
{
    ...
    mipi_dsi_brg_ll_clear_interrupt_status(hal->bridge, error_status);
        ...
        // fcipaq: if a buffer unterrun of the MIPI tx buffer occured, the MIPI driver will issue a 
        // vsync event and the whole frame has to be scanned out again.
        // telling the scaler via "returnvalue": 0: no error; 1: vsync
        // TODO: make this into a nice structure
        returnvalue |= 0b100000000;
        linenums = 0;
    }
    ...
    for (int j = 0; j < dpi_panel->lcd_scaling_factor; j++) {
        if (j == dpi_panel->lcd_scaling_factor - 1)
          markers.is_last = true;
        dw_gdma_lli_set_block_markers(dw_gdma_link_list_get_item(link_list, j), markers);
    }
...
//        if (dpi_panel->on_refresh_done(&dpi_panel->base, NULL, dpi_panel->user_ctx)) {
        if (dpi_panel->on_refresh_done(&dpi_panel->base, NULL, (void*) returnvalue)) {
            linenums++;
            if (linenums == dpi_panel->v_pixels / dpi_panel->lcd_scaling_factor) {
              linenums = 0;
              yield_needed = true;
            }
        }
...
static esp_err_t dpi_panel_create_dma_link(esp_lcd_dpi_panel_t *dpi_panel)
{
    ...
    // fcipaq: Use the DMA to scan out each scanline dpi_panel->lcd_scaling_factor times for nearest neighbor scaling
    dw_gdma_link_list_config_t link_list_config = {        
        .num_items = dpi_panel->lcd_scaling_factor,  // fcipaq: from mipi_dsi_priv.h: "NOTE: we assume 1 DMA link item can carry the WHOLE image"
...
esp_err_t esp_lcd_new_panel_dpi(esp_lcd_dsi_bus_handle_t bus, const esp_lcd_dpi_panel_config_t *panel_config, esp_lcd_panel_handle_t *ret_panel, int lcd_scaling)
{
    ...
    dpi_panel->lcd_scaling_factor = lcd_scaling;

    ESP_LOGI(TAG, "Allocating %d framebuffers", num_fbs);
...
//    size_t fb_size = panel_config->video_timing.h_size * panel_config->video_timing.v_size * bits_per_pixel / 8;
    size_t fb_size = panel_config->video_timing.h_size *  bits_per_pixel / 8;
    uint8_t *frame_buffer = NULL;
    for (int i = 0; i < num_fbs; i++) {
        frame_buffer = heap_caps_aligned_calloc(alignment, 1, fb_size, MALLOC_CAP_DMA | MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL);
    ...
    // fcipaq: enable TE
    mipi_dsi_host_ll_enable_te_ack(hal->host, 1);
    ...
    //mipi_dsi_brg_ll_set_underrun_discard_count(hal->bridge, panel_config->video_timing.h_size * 700); // fcipaq
...
static esp_err_t dpi_panel_init(esp_lcd_panel_t *panel)
{
    ...
    for (int i = 0; i < dpi_panel->num_fbs; i++) {
        ...
        // fcipaq: program the DMA to send each framebuffer dpi_panel->lcd_scaling_factor times
        for (int j = 0; j < dpi_panel->lcd_scaling_factor; j++)
          dw_gdma_lli_config_transfer(dw_gdma_link_list_get_item(link_list, j), &dma_transfer_config);
    ...
    // enable TE
    //mipi_dsi_host_ll_enable_te_ack(hal->host, true);  // fcipaq
